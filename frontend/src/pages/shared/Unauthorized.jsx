    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50">
      <div className="max-w-md w-full bg-white shadow-lg rounded-lg p-8 text-center">
        <div className="mb-6">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">{t('errors.accessDenied')}</h3>
          <p className="text-sm text-gray-500 mb-4">
            {t('errors.noPermission', "You don't have permission to access this page.")}
          </p>
          {requiredRoles.length > 0 && (
            <div className="text-xs text-gray-400 mb-4">
              <p>{t('errors.requiredRoles', 'Required role(s)')}: {requiredRoles.join(', ')}</p>
              {userRole && <p>{t('errors.yourRole', 'Your role')}: {userRole}</p>}
            </div>
          )}
        </div>
