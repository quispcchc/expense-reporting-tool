...
function ClaimNotes({ curClaim, mode, toastRef }) {
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-sm w-full h-full flex flex-col">
            <div className="mb-4">
                <h5 className="text-xl font-semibold text-gray-800 mb-1">Notes</h5>
                <p className="text-sm text-gray-500">Review notes and comments for this claim.</p>
            </div>
            {notes && notes.length > 0 ?
                (notes.map((note, index) => (
                    <Note key={index} description={note.claim_note_text} date={note.created_at}
                        submittedBy={note.user.full_name} />))) : (<p className="text-gray-500 text-sm mb-4">No notes available</p>)
            }
